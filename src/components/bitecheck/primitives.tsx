...
const chipPalette: Record<Tone, { bg: string; ink: string; border: string }> = {
  neutral: {
  ...
  return (
    <div
      ...
      style={{
        ...
        background: palette.bg,
        color: palette.ink,
        border: tone === 'neutral' ? '1px solid var(--bc-hairline)' : '1px solid transparent',
      }}
    >
      <span
        style={{
...
type ButtonKind = 'primary' | 'secondary' | 'soft' | 'ghost' | 'warnSoft' | 'safeSoft';
...
const buttonKinds: Record<ButtonKind, { bg: string; ink: string; border: string; shadow: string }> =
  {
    primary: {
      bg: 'var(--bc-primary)',
      ink: 'var(--bc-text-inv)',
...
export function ConfidenceMeter({
  ...
  return (
    <div className={className} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
      ...
      <div className="bc-meta" style={{ color: map.ink, fontWeight: 600 }}>
        {map.label}
      </div>
    </div>
  );
...
export function ThinkingDots({ className }: { className?: string }) {
  return (
    <div className={className} style={{ display: 'inline-flex', gap: 3, alignItems: 'center' }}>
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          style={{
