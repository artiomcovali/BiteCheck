...
const dietaryPreferenceSchema = z.enum(
  DIETARY_PREFERENCE_OPTIONS.map((option) => option.value) as [string, ...string[]],
);
...
const religiousRestrictionSchema = z.enum(
  RELIGIOUS_RESTRICTION_OPTIONS.map((option) => option.value) as [string, ...string[]],
);
const severitySchema = z.enum(
  SEVERITY_OPTIONS.map((option) => option.value) as [string, ...string[]],
);
const schoolSchema = z.enum(SCHOOL_OPTIONS.map((option) => option.value) as [string, ...string[]]);
...
const requiredBalance = z.preprocess(
  (value) => {
    if (typeof value !== 'string') return value;
    const trimmed = value.trim();
...
const profileSchema = z.object({
  ...
  polycard_balance: requiredBalance,
...
export async function updateProfileAction(
  ...
  if (authError || !user) {
    return { error: 'Sign in again to update your profile.' };
  }
  ...
  // Revalidate all pages so the layout re-fetches the profile
  // (updates the ProfilePill name/balance in the nav)
